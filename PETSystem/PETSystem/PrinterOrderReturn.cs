...
namespace PETSystem
{
    public partial class PrinterOrderReturn : Form
    {
        ...
        private void txtPrinterOrderID_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                dgvSuppOrder.DataSource = null;
                var LoadOrders = from PrinterOrder in db.PrinterOrders select PrinterOrder;
                dgvSuppOrder.DataSource = LoadOrders;
                dgvSuppOrder.Update();
                dgvSuppOrder.Refresh();
            }
        ...
        {
            if (dgvSuppOrder.SelectedCells.Count > 0)
            {
                PrinterOrder _Printer = (PrinterOrder)dgvSuppOrder.CurrentRow.DataBoundItem;

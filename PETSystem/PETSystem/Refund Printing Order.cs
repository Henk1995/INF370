...
namespace PETSystem
{
    public partial class Refund_Printing_Order : Form
    {
        public Refund_Printing_Order()
        {
            InitializeComponent();
        }
        int stop = 0;
        ...
        private void Refund_Printing_Order_Load(object sender, EventArgs e)
        {
            ...
            label1.Text = "Printer ID:" + ConnectString.SupplierID + "\nPrinter Name: " + ConnectString.SupplierName;
                    ...
                    using (SmtpClient client = new SmtpClient())
                    {
                        ...
                        using (MailMessage mail = new MailMessage("janwilkensmalan1@gmail.com", emailA))
                        {
                            try
                            {
                                ...
                                this.Close();
                                this.Dispose(true);
                                MessageBox.Show("Supplier notified via email that Order with \nReference number: " + RefID + "\nIs being returned\nReturning to printing suppliers screen.", "Sent", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                                Search_Printing_Supplier ff = new Search_Printing_Supplier();
                                ff.ShowDialog();
        ...
        private void txtSupplierOrderID_TextChanged(object sender, EventArgs e)
        {
            ...
            else if (comboBox1.SelectedIndex == 1)
            {
                ...
                DataTable DT = new DataTable();
                ConnectString.connectstring.Open();
                SqlCommand Fill = new SqlCommand("SELECT PrinterOrder.PrinterOrderRefNumber,PrinterOrder.PrintOrderDate,PrinterOrder.PrintOrderDescription from PrinterOrder Where PrinterOrder.PrinterID = '" + ConnectString.SupplierID + "' AND PrintOrderDate Like '%" + txtSupplierOrderID.Text + "%'", ConnectString.connectstring);
                DA = new SqlDataAdapter(Fill);
                DA.Fill(DT);
                dgvSupp.DataSource = DT;
                dgvSupp.DataMember = DT.TableName;
            ...
            else
            {
                ...
                DataTable DT = new DataTable();
                ConnectString.connectstring.Open();
                SqlCommand Fill = new SqlCommand("SELECT PrinterOrder.PrinterOrderRefNumber,PrinterOrder.PrintOrderDate,PrinterOrder.PrintOrderDescription from PrinterOrder Where PrinterOrder.PrinterID = '" + ConnectString.SupplierID + "' AND PrintOrderDescription Like '%" + txtSupplierOrderID.Text + "%'", ConnectString.connectstring);
                DA = new SqlDataAdapter(Fill);
                DA.Fill(DT);
                dgvSupp.DataSource = DT;
                dgvSupp.DataMember = DT.TableName;
